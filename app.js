...
function renderProducts(list = products) {
  ...
  list.forEach((p, index) => {
    ...
    card.className =
      "product bg-gray-900 rounded-2xl p-4 border border-white/10 active-scale flex flex-col h-full";
    ...
    card.innerHTML = `
      ...
      <div class="flex gap-2 mt-auto">
        <button onclick="addToCart(${index})"
          class="flex-1 bg-yellow-500 text-black text-xs py-2 rounded-lg font-bold">
          Savatga
        </button>

        <button onclick="showDetails(${index})"
          class="flex-1 bg-white/10 text-xs py-2 rounded-lg">
          Batafsil
        </button>
      </div>
    `;
...
function addToCart(id) {
    ...
    alert(name + " " + translations[currentLang].added);

    // Telegramga ham yuborish
    sendProductToTelegram(item);
}
function updateAuthUI() {
  ...
  function renderCart() {
    ...
    if (cart.length === 0) {
        list.innerHTML = `<p class="text-center py-10 text-gray-500">${currentLang === 'uz' ? 'Savat bo\'sh' : 'Корзина пуста'}</p>`;
    ...
    cart.forEach((item, index) => {
        ...
        list.innerHTML += `
            <div class="flex items-center gap-4 bg-gray-800 p-3 rounded-2xl border border-white/5">
                ...
                <div class="flex-1 text-sm font-bold">${item.name}</div>
